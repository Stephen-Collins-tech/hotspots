...
async function runFaultline(binaryPath, inputs, context) {
    ...
    const args = ['analyze', inputs.path, '--mode', 'snapshot'];
    if (inputs.minLrs) {
...
async function runPRAnalysis(binaryPath, inputs) {
    ...
    if (inputs.config)
        snapshotArgs.push('--config', inputs.config);
    if (inputs.minLrs)
        snapshotArgs.push('--min-lrs', inputs.minLrs);
    ...
    if (baseSha) {
        ...
        if (inputs.policy)
            diffArgs.push('--policy');
        if (inputs.config)
            diffArgs.push('--config', inputs.config);
        core.info(`Running: ${binaryPath} ${diffArgs.join(' ')}`);
        let diffStdout = '';
        let diffStderr = '';
        const diffExitCode = await exec.exec(binaryPath, diffArgs, {
            listeners: {
                stdout: (data) => { diffStdout += data.toString(); },
                stderr: (data) => { diffStderr += data.toString(); }
            },
            ignoreReturnCode: true
        });
        if (diffExitCode === 3) {
            ...
            if (inputs.policy)
                htmlArgs.push('--policy');
            if (inputs.config)
                htmlArgs.push('--config', inputs.config);
            ...
            const violations = [
                ...policy.failed.map((v) => ({ ...v, level: 'error' })),
                ...policy.warnings.map((v) => ({ ...v, level: 'warning' }))
            ];
...
async function runDeltaAnalysis(binaryPath, inputs) {
    const args = ['analyze', inputs.path, '--mode', 'delta'];
    if (inputs.policy)
        args.push('--policy');
    if (inputs.minLrs)
        args.push('--min-lrs', inputs.minLrs);
...
function generateSummary(result, context) {
    const violations = result.violations || [];
    const errors = violations.filter((v) => v.level === 'error');
    const warnings = violations.filter((v) => v.level === 'warning');
...
function generateDiffSummary(result, baseSha, headSha) {
    const deltas = result.deltas || [];
    const policy = result.policy || { failed: [], warnings: [] };
    const newFns = deltas.filter((d) => d.status === 'new');
    const modified = deltas.filter((d) => d.status === 'modified');
    const deleted = deltas.filter((d) => d.status === 'deleted');
    let summary = '# Hotspots Analysis Results\n\n';
    ...
    if (policy.failed.length > 0) {
        ...
        policy.failed.slice(0, 10).forEach((v) => {
            summary += `- ${v.message}\n`;
        });
        if (policy.failed.length > 10)
            summary += `\n*...and ${policy.failed.length - 10} more*\n`;
        summary += '\n';
    }
    if (policy.warnings.length > 0) {
        summary += '## ⚠️ Warnings\n\n';
        policy.warnings.slice(0, 10).forEach((v) => {
            summary += `- ${v.message}\n`;
        });
        if (policy.warnings.length > 10)
            summary += `\n*...and ${policy.warnings.length - 10} more*\n`;
        summary += '\n';
    }
    return summary;
}
async function postPRComment(token, summary, reportPath) {
