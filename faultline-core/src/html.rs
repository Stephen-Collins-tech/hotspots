...
<head>
    ...
    <title>Faultline Report - {sha}</title>
    <style>{css}</style>
</head>
<body>
    <div class="container">
    ...
    <script>{js}</script>
        ...
        aggregates_section = aggregates.map(|a| render_aggregates(a)).unwrap_or_default(),
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Faultline Delta Report - {sha}</title>
    <style>{css}</style>
</head>
<body>
    <div class="container">
...
fn render_delta_table(deltas: &[FunctionDeltaEntry]) -> String {
    ...
    <td>{status_display}</td>
                ...
                status_display = format!("{:?}", entry.status),
            )
        })
        .collect();
